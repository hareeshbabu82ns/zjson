...
public section.
  ...
  constants:
    BEGIN OF pretty_mode,
        ...
        END OF  pretty_mode .
  class-data SV_WHITE_SPACE type STRING .
  ...
  class-methods ESCAPE
    ...
    returning
      value(OUT) type STRING .
  class-methods SERIALIZE_TO_OBJ
    importing
      !IV_JSON type STRING
    returning
      value(RV_OBJECT) type STRING .
  class-methods SERIALIZE_ARRAY_TO_OBJ
    importing
      !IV_ARRAY type STRING
    returning
      value(RV_OBJECT) type STRING .
protected section.
...
METHOD serialize_array_to_obj.
  rv_object = iv_array.
  SHIFT rv_object LEFT DELETING LEADING space.
  IF rv_object(1) = '['.
    CONCATENATE `{"data": ` rv_object `}` INTO rv_object.
  ENDIF.
ENDMETHOD.


METHOD serialize_to_obj.

  rv_object = iv_json.
  CONCATENATE `{` rv_object `}` INTO rv_object.
